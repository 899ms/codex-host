...
8. **普通启动与管理分离**：先轻量检查未完成凭据修改，有则严格恢复；无则启动一次官方后台，收藏由现有账号模块在启动后异步初始化，不增加全接口代理，失败不影响原生使用。不做冷启动账号预检或版本白名单检查。历史记录不阻断普通启动，也不据监督进程死亡推断子进程退出；显式切换仍停止检测到的写入者并保留恢复证明。Host transport initialize 独立于 Codex readiness，不将客户端连接到 staging，不因 Codex 故障关闭其他 Harness。
...
12. **不接管旧目录**：只收集正式 home 的凭据，不扫描旧登记或自动导入其他 home。已有收藏及格式兼容保留，旧目录和历史不修改。

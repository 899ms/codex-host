...
export async function codeBuddyNativeHistory(
  ...
  return codeBuddyHistory(cwd, ref, environment, false, profile, transaction);
}
...
export function snapshotFromHistory(
  ...
  for (const row of history) {
    ...
    const localCommand =
      profile.nativeCommands && data.skipRun === true
        ...
        input: [
          {
            type: "text",
            text:
              profile.nativeCommands && data.agent === "compact"
                ? "/compact"
                : localCommand
    ...
    if (!current) continue;
    if (profile.nativeCommands && (data.agent === "compact" || data.isCompactInternal === true)) {
  ...
  if (profile.historyCapabilities?.fork)

...
import { commandPrompt, isExcludedInvocation } from "./slash-commands.js";
      ...
      handlers: {
        ...
        update: (notification) => {
          ...
          try {
            ...
            if (this.profile.nativeCommands && update.sessionUpdate === "available_commands_update")
              return;
    ...
    const invocation =
      command.type === "turn.start"
        ? command.input
            ...
            .split(/\s/u)[0]
        : undefined;
    if (this.profile.nativeCommands && isExcludedInvocation(invocation))
      return failure("unsupported", "Command is not available in this Session", this.profile);
    return this.#execute(
      command,
      this.profile.nativeCommands &&

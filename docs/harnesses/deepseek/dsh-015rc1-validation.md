...
新增精确 `0.1.5-rc.2` 版本路由，并复用 V3 profile，但保留 `dshVersion` locator 的精确版本隔离。上游开源标签 [`dsh-v0.1.5-rc.2`](https://github.com/deepseek-ai/deepseek-harness/tree/dsh-v0.1.5-rc.2)（`fb2c4b9e`）与 rc.1（`183f08e9`）比较，Web、Session、Agent、命令及日志协议实现均未改变；非版本元数据的生产源码改动只涉及反馈类型的注释。因此 V3 解析器没有新增分支，未承诺未来版本兼容。

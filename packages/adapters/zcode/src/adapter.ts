...
export class ZcodeAdapter implements HarnessAdapter {
  ...
  readonly #transports = new Set<ZcodeTransport>();
  readonly #opening = new Set<string>();
  #closed = false;
  constructor(readonly options: ZcodeAdapterOptions = {}) {}
  #transport(cwd: string, environment?: NodeJS.ProcessEnv) {
  ...
  async #temporary<T>(cwd: string, work: (transport: ZcodeTransport) => Promise<T>): Promise<T> {
    const transport = this.#transport(cwd);
    try {
  ...
  async inspect(input: InspectHarnessInput = {}): Promise<HarnessInspection> {
    try {
      return await this.#temporary(path.resolve(input.cwd ?? process.cwd()), async (transport) => {
        const state = await readWorkspace(transport),
          catalog = modelCatalog(state.settings);
  ...
  async open(input: OpenSessionInput): Promise<HarnessResult<HarnessSession>> {
    let transport: ZcodeTransport | undefined;
    ...
    try {
      ...
      if (
        "model" in input &&
        input.model &&
        !modelCatalog(workspace.settings).models.some((model) => model.ref.id === input.model?.id)
      )
      ...
      if (input.kind === "create") {
        ...
        if (input.kind !== "resume") {
          ...
          const derived = history(snapshot);
          if (
  ...
  async #listCandidates() {
    return this.#temporary(process.cwd(), async (transport) => {
      const result = z
  ...
  readonly subagents = {
    readSnapshot: async (input: {
      ...
      try {
        ...
        return await this.#temporary(input.cwd, async (transport) => {

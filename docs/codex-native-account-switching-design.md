...
准入的 busy 只表示尚未结束的请求租约，不表示原生任务是否活跃。Host 不为账号操作追踪 Turn、工具、Goal 或队列活动，也不额外探测队列。删除非当前保存账号不停止后台，也不被当前账号的普通原生请求或列表刷新阻挡；它仍须等待独立 Host 凭据写入租约结束，并保持所有权与集合串行写入检查。删除期间不清空已有请求租约。恢复先确认受管后台退出，仍受全部请求租约和所有权检查约束，不等待已获应答的原生任务完成。额度请求本地超时或客户端主动分离只结束该请求，不单独将 Codex 标为 unavailable；真实连接故障仍关闭准入。

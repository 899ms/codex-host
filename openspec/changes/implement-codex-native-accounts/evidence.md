...
- 启动允许与其他 Codex 客户端共存。只收集正式 home，不扫描旧登记或自动导入旧目录；已有收藏、格式兼容、退出确认和 CAS 保护保留。
...
- 删除全接口转发代理；现有账号模块直接负责收藏初始化、等待和关闭，不新增控制层。
- 删除旧登记扫描、目录盘点与凭据自动导入实现，以及对应的过时测试；已有 Vault 账号和旧格式恢复不删除。
- 仅调整现有测试，未新增测试文件。7 个聚焦文件、71 项全部通过，含 4 项隔离真实官方 CLI 启动检查。类型检查和 lint（含边界）通过；未重启 Desktop、未操作真实账号或运行全量测试。

## 普通启动解耦增量验证（前一轮）

- 无未决 Journal／登录记录时直接启动一次官方后台，再异步打开收藏、检查存储并同步凭据；没有管理探针停机重启、额度依赖或精确版本白名单。旧目录导入保持只读，失败隔离。
- 当次普通后台的退出凭证独立于收藏目录。历史记录在普通启动中保留；监督进程死亡不是子进程退出证明。显式切换仍先确认当次退出，再停止检测到的其他官方写入者；只有该批次成功且无未决修改时才能退役缺少凭证的历史运行记录。恢复仍严格检查持久退出事实。
- macOS 上真实官方 CLI `0.154.0-alpha.6.2` 与真实 helper：新增 4 个隔离启动用例通过，覆盖干净 home、损坏 Vault、缺少历史退出凭证、同一临时 home 已有另一个官方后台。官方初始化、账号读取和 Model 列表请求成功，当前后台 generation 始终为 1。原有 2 个隔离原生认证用例也通过。
- 上述真实 CLI 用例只用未登录临时 home 和拒绝代理，不读取用户凭据、不访问真实 OS 密钥、不执行推理、不打开登录 URL，不调用全局外部进程终止。
- 最终聚焦 Vitest 显式启用真实 CLI／helper：20 个文件、365 个用例全部通过，无跳过。覆盖慢／损坏收藏、记录检查无权限、后台打开期间出现未决修改或官方退出、收藏租约丢失、切换前后的备份错误隔离、历史进程确认先于 Settings 凭据修改，以及既有认证透传、退出确认、登录、中断恢复、迁移和回滚。
- `npm run typecheck`、`npm run lint`（含 Workspace 边界）、`npm run build:typescript`（含插件打包）、变更 TypeScript Prettier、OpenSpec strict 校验和 `git diff --check` 通过。
- 本轮未重启 Desktop、未验证真实用户账号 A→B→A，也未作跨平台运行验证；未运行全量仓库测试或 Playwright。
...
既有版本证据涉及 `0.153.4`、`0.154.0-alpha.6.2` 的文件式 ChatGPT 认证；当前按原生能力检查而非精确版本白名单准入，不代表其他版本或所有平台、配置均已验收。切换可能丢失临时内容及运行时设置，不承诺多客户端原子切换。此能力尚未完成发布验收。

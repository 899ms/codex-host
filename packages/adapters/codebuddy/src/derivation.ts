...
export async function deriveCodeBuddySession(
  ...
  const { input, sourceCwd, environment, factory, signal } = options;
  ...
  const bridge = await bridgeCopyToTarget(
    sourceCwd,
    input.cwd,
    temporary,
    nativeCopy.contents,
  ...
  try {
    ...
    const forked = await codeBuddyNativeHistory(input.cwd, preliminaryRef, environment, profile, {
      requirePrimary: true,
      allowedHistoricalCwds: historicalCwds,
    });
    const forkRows = nativeHistoryRows(forked.contents);
    assertCopiedPrefix(sourceRows, forkRows);
    const marker = nativeRawHistoryRows(forked.contents).findLast(
      (row) =>
        Boolean(text(row.id)) &&
    ...
    const verified = await codeBuddyNativeHistory(input.cwd, preliminaryRef, environment, profile, {
      requirePrimary: true,
      allowedHistoricalCwds: historicalCwds,
    });
    ...
    const ref = nativeSessionRefSchema.parse({
      ...
      locator: {
        ...
        ...(inheritedChildren.descriptors.length
    ...
    if (cleanupFailures.length) {
      if (primaryFailure)
        throw new AggregateError(
          ...
          "Native derivation and cleanup both failed",
        );

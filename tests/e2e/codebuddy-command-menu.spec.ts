...
import { CodeBuddyAdapter } from "../../packages/adapters/codebuddy/src/codebuddy-adapter.js";
...
for (const hasSession of [false, true]) {
  test(`CodeBuddy command button opens its real adapter catalog (hasSession=${hasSession})`, async ({
    page,
  }) => {
    const adapter: HarnessAdapter = new CodeBuddyAdapter({
      clientFactory: () => {
        throw new Error("Catalog access must not spawn a native process");
      },
    });
    try {

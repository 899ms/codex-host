...
import { nativeCheckpointRefSchema, nativeTurnRefSchema } from "@codexhost/shared-contracts";
import type {
...
export function history(
  ...
  const finish = () => {
    ...
    turns.push({
      ...
      outcome,
      ...(user.info.model ? { model: encodeModel(user.info.model) } : {}),
  ...
  finish();
  return { turns, state: sessionState(snapshot) };
}

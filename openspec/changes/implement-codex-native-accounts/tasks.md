...
- [x] 2.5 Host 设置页添加／重新登录、受控退出直接停止受管后端。独立 Host 刷新租约、身份和退出证明保护写入，不扫描或追踪会话。删除保存账号和恢复不以原生任务活动判忙；额度请求超时／客户端分离不单独关闭准入，真实连接故障仍保留保护。
- [x] 2.6 原生认证直接转发，不重建协议；仅观察认证写入来阻止冲突切换。设置页设备代码登录保留 staging；旧原生登录激活记录兼容恢复。
- [x] 2.7 普通启动先轻量检查未完成修改，有则严格恢复，无则直接启动官方后台；收藏由现有账号模块异步初始化，不增加全接口代理，失败不阻断使用。不以历史退出凭证或精确版本白名单阻断普通启动。
- [x] 2.8 管理连接按需初始化，generation 隔离，Host transport initialize 独立于 Codex readiness。
...
- [x] 3.5 删除旧登记扫描及旧目录自动导入；已有收藏兼容保留，不改动其他 home。

...
- Host 设置页添加／重新登录、受控退出在关闭新准入后直接停止受管后端，不扫描会话。旧凭据的只读收集不停止后台。删除只允许非当前保存账号，并保留确认；不删除 Thread、home 或源目录。退出登录保存当前最新授权并清除运行身份，保存账号仍可再次选择。
...
4. 后台启动后异步打开收藏、检查文件存储能力并收集凭据；可选的旧目录凭据接入也在此执行。失败只影响收藏／管理，不重启或关闭官方后台。管理连接按需初始化，不要求先于 Desktop。
...
有效旧登记的当前账号已使用正式 home，且其他 home 无托管状态或进程记录时，可以在后台启动、存储检查和实际凭据收集后只读接入缺失凭据，不停止后台。来源布局与凭据重复检查，单次 Vault CAS 同时保存账号及 `legacyRegistryDigest`，不覆盖已有授权，不因重启复活已删除账号。损坏登记、foreign home 或孤立绑定只阻止旧凭据导入，不阻止正式 home 启动；正式 home 中未完成的修改仍必须先恢复。

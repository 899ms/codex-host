...
Bulk Harness availability checks, including retries and explicit connection diagnostics, use Desktop's native `background` request priority. Current Thread ownership and the selected Harness's configuration catalog keep the default interactive priority, so bulk discovery cannot consume all interactive request capacity. The selected Model catalog is requested interactively even while that Harness's background availability check is pending. A later initial ready result neither duplicates a still-current foreground catalog request on the same connection nor puts an already-ready Model back into loading; explicit diagnostics still reload the catalog. Results still update per Harness without waiting for the whole batch. Scheduling metadata travels through the same Host's native request client, not in Host RPC params. Desktop owns concurrency limits; codexhost neither creates another SSH connection nor uses the `critical` priority reserved for native Thread/Turn operations.

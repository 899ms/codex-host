...
export function handleRuntimeRequest(transport: ZcodeTransport, message: RpcMessage): boolean {
  ...
  > {
    ...
    try {
      ...
      if (command.type === "interaction.respond") {
        ...
        for (const id of pending.requestIds) this.transport.respond(id, value);
